...
public interface AgendamentoRepository extends JpaRepository<Agendamento, Long>{
	...
	@Query(value = "SELECT a FROM agendamento a  WHERE a.cliente_id = :id AND a.data_agendamento = :dataAgendamento" , nativeQuery = true)
	Agendamento consultaAgendamento(@Param("id") Long id, @Param("dataAgendamento")Timestamp dataAgendamento );

	@Query(value = "SELECT a FROM agendamento a  WHERE a.cliente_id = :id " , nativeQuery = true)
	Agendamento consultaAgendamentoCliente(@Param("id") Long id);
	
	@Query(value = "SELECT a FROM agendamento a  WHERE a.cliente_id = :id AND a.id = :idAgendamento" , nativeQuery = true)
